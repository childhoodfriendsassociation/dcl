...
<body>
    ...
    <div class="container">
        ...
        <!-- ================= PAGE: BANK & ASSETS ================= -->
        <div id="bank-page" class="view-panel">
            <div class="data-card">
                <h3 style="margin-bottom: 12px; color: #0f172a;">ব্যাংক লেনদেন ও ডেন্টাল ইকুইপমেন্ট ক্রয়</h3>
                <p style="margin-bottom: 12px; font-size: 13px; color: #475569;">
                    <strong>ব্যাংক অ্যাকাউন্ট:</strong> Dent Craft Lab Operations | <strong>ব্যাংক:</strong> Mutual Trust Bank
                </p>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <tr>
                            <td>০৫/০৯/২০২৬</td>
                            <td>ডেন্টাল মাইক্রোমোটর ও টুলস ক্রয়</td>
                            <td>ব্যাংক ট্র্যান্সফার</td>
                            <td class="text-right">৬০,০০০</td>
                        </tr>
                        <tr>
                            <td>১০/০৯/২০২৬</td>
                            <td>মেডিকেল গ্রেড ডেন্টাল সিরামিক ও রেজিন</td>
                            <td>নগদ (Cash)</td>
                            <td class="text-right">২৫,০০০</td>
                        </tr>
                        <tr class="total-row">
                            <td colspan="3" class="text-right">মোট খরচের পরিমাণ:</td>
        ...
        <!-- ================= PAGE: PROFIT & LOSS ================= -->
        <div id="profit-page" class="view-panel">
            <div class="data-card">
                <h3 style="margin-bottom: 16px; color: #0f172a;">Dent Craft Lab - নিট মুনাফা বিশ্লেষণ</h3>
                <table>
                    <tbody>
                        <tr>
                            <td><strong>মোট সার্ভিস আয় (Lab Revenue)</strong></td>
                            <td class="text-right" style="font-weight: bold; font-size: 15px;">৳ ২,৪৫,০০০</td>
                        </tr>
                        <tr>
                            <td><strong>বাদ: পরিচালন ও ইকুইপমেন্ট খরচ (Operating Expenses)</strong></td>
                            <td class="text-right" style="font-weight: bold; color: #dc2626; font-size: 15px;">(-) ৳ ৮৫,০০০</td>
                        </tr>
                        <tr class="total-row" style="font-size: 15px;">
                            <td><strong>খুচরা নিট মুনাফা (Net Profit)</strong></td>
                            <td class="text-right" style="font-weight: bold; color: #16a34a;">৳ ১,৬০,০০০</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <footer>
      <div>
         <div>Dent Craft Lab — New Circular Road, Barishal Sadar, Barishal | Precision Dental Solutions </div>
         <div>© 2026, Dent Craf  Lab</div>
        <div>Developer: Abdul Awal, Dubai, UAE</div>
          </div>
    </footer>

    <!-- Live Clock & Calendar Logic -->
    <script>
        let lastRenderedDate = null;

        // Function to update Live Clock & Render Calendar dynamically
        function updateLiveClockAndCalendar() {
            const now = new Date();

            // 1. Live Clock Update (Bengali Numerals Format)
            document.getElementById('current-time-text').innerHTML = `<i class="fa-regular fa-clock"></i> ${now.toLocaleTimeString('bn-BD')}`;

            // 2. Calendar Auto-Refresh Logic (Checks if day or month changed)
        ...
        function renderCalendar(now) {
            ...
            const dayHeadings = ["রবি", "সোম", "মঙ্গল", "বুধ", "বৃহঃ", "শুক্র", "শনি"];
            ...
            dayHeadings.forEach(day => {
                const dayHeadDiv = document.createElement('div');
                dayHeadDiv.className = 'calendar-day-head';
                dayHeadDiv.innerText = day;
                container.appendChild(dayHeadDiv);
            });
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                dayDiv.className = 'calendar-day' + (day === todayDate ? ' today' : '');
        ...
        function handleLogin(e) {
            ...
            if (memberName && pin) {
                document.getElementById('user-status-header').innerHTML = `
                    <strong style="display:block;">${memberName}</strong>
                    <span style="font-size: 11px; color: #38bdf8;">সক্রিয় সদস্য</span>
                `;
                openPage('home-dashboard');
            }
        }
        ...
        function openPage(pageId) {
            ...
            if (pageId === 'login-page') {
                ...
                pageTitle.innerText = "সদস্য সিকিউরিটি লগইন";
            } else if (pageId === 'home-dashboard') {
                backBtn.style.display = 'none';
                pageTitle.innerText = "মূল ড্যাশবোর্ড (Home Dashboard)";
            } else {
                backBtn.style.display = 'inline-block';
                if (pageId === 'daily-ledger-page') pageTitle.innerText = "ডেইলি লেজার (Daily Income)";
                else if (pageId === 'daily-expense-page') pageTitle.innerText = "ডেইলি এক্সপেন্স (Daily Expenses)";
                else if (pageId === 'overview-page') pageTitle.innerText = "ল্যাব ওভারভিউ";
                else if (pageId === 'members-page') pageTitle.innerText = "সদস্য ও শেয়ার রেজিস্টার";
                else if (pageId === 'bank-page') pageTitle.innerText = "ব্যাংক ও ম্যাটেরিয়াল খরচ";
                else if (pageId === 'profit-page') pageTitle.innerText = "লাভ-ক্ষতির হিসাব";
            }
        }

        // Initialize Live Clock and Start Interval (Every 1 Second)
        updateLiveClockAndCalendar();
        setInterval(updateLiveClockAndCalendar, 1000);
    </script>
</body>
</html>
